...
if (version_compare(phpversion(), '5.3.1', '<')) {
    if (substr(phpversion(), 0, 5) != '5.3.1') {
        ...
        echo "PEAR2_ReleaseBroker requires PHP 5.3.1 or newer.
";
        exit -1;
    }
}
...
try {
    ...
    echo "Cannot process PEAR2_ReleaseBroker phar:
";
    echo $e->getMessage(), "
";
    exit -1;
}
function PEAR2_ReleaseBroker_autoload($class)
{
    $class = str_replace(array('_', '\\'), '/', $class);
    if (file_exists('phar://' . __FILE__ . '/PEAR2_ReleaseBroker-@PACKAGE_VERSION@/php/' . $class . '.php')) {
        include 'phar://' . __FILE__ . '/PEAR2_ReleaseBroker-@PACKAGE_VERSION@/php/' . $class . '.php';
    }
}
spl_autoload_register("PEAR2_ReleaseBroker_autoload");
$phar = new Phar(__FILE__);
$sig = $phar->getSignature();
define('PEAR2_ReleaseBroker_SIG', $sig['hash']);
define('PEAR2_ReleaseBroker_SIGTYPE', $sig['hash_type']);
